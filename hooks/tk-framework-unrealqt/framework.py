...
class UnrealQtFramework(sgtk.platform.Framework):
    ...
    def init_framework(self):
        ...
        # Check if PySide is already available.
        # NOTE: In UE 5.5+ Unreal exposes its internal Qt but does not fully
        # populate sgtk.platform.qt (QtGui etc. come back as None). We therefore
        # skip the early-return and always activate the bundled PySide6 vendor
        # so that shotgunutils and other apps get a complete Qt binding.
        try:
            from sgtk.platform.qt import QtGui  # noqa
            if QtGui is not None and hasattr(QtGui, "QStandardItem"):
                self.logger.debug("Qt is fully available, not activating any custom package.")
                return
            else:
                self.logger.debug("Qt is partially available (UE 5.5+), activating custom package.")
